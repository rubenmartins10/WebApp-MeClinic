...
import { Users, Search, User, Phone, Mail, FileText, Calendar, Save, X, Activity, Clock, CheckCircle, XCircle, File, Download, UploadCloud, Trash2, AlertTriangle, Eye, Edit2, Pill, Plus, Minus, MessageCircle } from 'lucide-react';
...
const Pacientes = () => {
  ...
  // ESTADOS DA RECEITA MÉDICA
  const [showRxModal, setShowRxModal] = useState(false);
  const [rxMeds, setRxMeds] = useState([{ nome: '', posologia: '' }]);
  const [sendWhatsapp, setSendWhatsapp] = useState(true); // <-- NOVO: Estado para o WhatsApp
    ...
    reader.onloadend = async () => {
      ...
      try {
        const res = await fetch(`/api/pacientes/${selectedPaciente.id}/exames`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ nome: file.name, base64: base64 })
        });
        if (res.ok) {
      ...
      } catch (err) { showNotif('Erro ao guardar documento.', 'error'); }
    };
  ...
  const apagarExame = async (idExame) => {
    try {
      ...
      if (res.ok) {
        showNotif('Documento apagado.');
        setExames(exames.filter(e => e.id !== idExame));
      }
    } catch (err) { showNotif('Erro ao apagar.', 'error'); }
  };
  ...
  const salvarEdicao = async (e) => {
    ...
    if (partesNome.length < 2) {
      showNotif('É obrigatório preencher o Primeiro e Último nome.', 'error'); return;
    }
    if (!formData.telefone || formData.telefone.replace(/\s+/g, '').length < 9) {
      showNotif('O número de telemóvel é obrigatório.', 'error'); return;
    }
    try {
      ...
      if (res.ok) {
        showNotif('Dados atualizados!');
        setShowEditModal(false); carregarPacientes();
        setSelectedPaciente({ ...selectedPaciente, nome: formData.nome, telefone: formData.telefone, email: formData.email });
      }
    } catch (err) { showNotif('Erro no servidor.', 'error'); }
  };

  // ===============================================
  // GERADOR MÁGICO DE RECEITAS COM WHATSAPP
  // ===============================================
  const adicionarMed = () => setRxMeds([...rxMeds, { nome: '', posologia: '' }]);
  
  const removerMed = (index) => {
    const novos = [...rxMeds];
  ...
  const preencherRapido = (nome, posologia) => {
    ...
    if (vazios.length > 0) {
      const novos = [...rxMeds];
      novos[rxMeds.length - 1] = { nome, posologia };
      setRxMeds(novos);
    } else {
      setRxMeds([...rxMeds, { nome, posologia }]);
    }
  };
  ...
  const gerarReceitaPDF = async () => {
    ...
    doc.text("MECLINIC", 20, 20);

    doc.setTextColor(0, 0, 0);
    doc.setFontSize(14); doc.text(t('patients.rx.title') || "RECEITA MÉDICA", 20, 32);
    ...
    rxMeds.forEach((med, index) => {
      ...
      if(med.nome) {
        ...
        doc.text(`${index + 1}. ${med.nome}`, 20, y);
        y += 6;
        doc.setFontSize(10); doc.setFont(undefined, 'normal');
        doc.text(med.posologia || 'Tomar conforme indicação médica.', 25, y);
        y += 14;
      }
    });
    ...
    try {
      ...
      // MAGIA: ABRIR O WHATSAPP COM A MENSAGEM PREPARADA
      if (sendWhatsapp && selectedPaciente.telefone) {
        // Limpa tudo o que não for número (ex: tira espaços, tira letras). Mantém o prefixo se já lá estiver.
      ...
      showNotif('Receita gerada, mas falha ao guardar no arquivo.', 'error');
    }
  };
  ...
  return (
    <div style={{ padding: '20px', color: theme.text, maxWidth: '1400px', margin: '0 auto', boxSizing: 'border-box' }}>
      ...
      {/* MODAL 1: GERADOR DE RECEITAS MÉDICAS */}
      {showRxModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.8)', zIndex: 1020, display: 'flex', justifyContent: 'center', alignItems: 'center', backdropFilter: 'blur(5px)', padding: '20px' }}>
          <div style={{ backgroundColor: theme.cardBg, width: '100%', maxWidth: '600px', borderRadius: '20px', padding: '35px', border: `1px solid ${theme.border}`, boxShadow: '0 25px 50px -12px rgba(0,0,0,0.5)', maxHeight: '90vh', display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
              ...
              <button onClick={() => { setShowRxModal(false); setRxMeds([{ nome: '', posologia: '' }]); }} style={{ background: 'none', border: 'none', color: theme.subText, cursor: 'pointer' }}><X size={24} /></button>
            </div>
            ...
            <div style={{ marginTop: '15px', backgroundColor: theme.isDark ? 'rgba(34, 197, 94, 0.1)' : '#f0fdf4', padding: '15px', borderRadius: '10px', border: `1px solid #22c55e` }}>
              ...
              {sendWhatsapp && (
                <p style={{ margin: '5px 0 0 28px', fontSize: '12px', color: theme.subText }}>
                  Abrirá o WhatsApp automaticamente. Só precisarás de arrastar a Receita para o chat.
                </p>
              )}
            </div>

            <div style={{ display: 'flex', gap: '10px', marginTop: '20px' }}>
              <button onClick={() => { setShowRxModal(false); setRxMeds([{ nome: '', posologia: '' }]); }} style={{ flex: 1, padding: '14px', borderRadius: '10px', border: 'none', backgroundColor: theme.pageBg, color: theme.text, fontWeight: 'bold', cursor: 'pointer' }}>Cancelar</button>
              <button onClick={gerarReceitaPDF} style={{ flex: 2, padding: '14px', borderRadius: '10px', border: 'none', backgroundColor: '#10b981', color: 'white', fontWeight: 'bold', cursor: 'pointer', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '10px', boxShadow: '0 4px 10px rgba(16, 185, 129, 0.3)' }}>
                <FileText size={18} /> {t('patients.rx.generate') || 'Gerar & Guardar Receita'}
              </button>
            </div>
          </div>
        </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', backgroundColor: theme.isDark ? '#1e293b' : '#f1f5f9' }}>
                <div style={{ display: 'flex', borderBottom: `1px solid ${theme.border}`, backgroundColor: theme.cardBg }}>
                  <button 
                    ...
                    style={{ flex: 1, padding: '20px', background: 'none', border: 'none', borderBottom: activeTab === 'historico' ? '3px solid #2563eb' : '3px solid transparent', color: activeTab === 'historico' ? '#2563eb' : theme.subText, fontWeight: 'bold', fontSize: '15px', cursor: 'pointer', transition: 'all 0.2s', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '8px' }}
                  >
                    <Calendar size={18} /> {t('patients.tabs.history')}
                  </button>
                  <button 
                    onClick={() => setActiveTab('exames')}
                    style={{ flex: 1, padding: '20px', background: 'none', border: 'none', borderBottom: activeTab === 'exames' ? '3px solid #2563eb' : '3px solid transparent', color: activeTab === 'exames' ? '#2563eb' : theme.subText, fontWeight: 'bold', fontSize: '15px', cursor: 'pointer', transition: 'all 0.2s', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '8px' }}
                  >
                        ...
                        return (
                          <div key={c.id} style={{ backgroundColor: theme.cardBg, padding: '15px', borderRadius: '12px', border: `1px solid ${theme.border}`, borderLeft: `4px solid ${isFinalizada ? '#10b981' : '#3b82f6'}`, boxShadow: '0 2px 4px rgba(0,0,0,0.02)' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                              ...
                              <span style={{ fontSize: '12px', color: isFinalizada ? '#10b981' : '#3b82f6', fontWeight: 'bold', backgroundColor: isFinalizada ? 'rgba(16,185,129,0.1)' : 'rgba(59,130,246,0.1)', padding: '4px 10px', borderRadius: '20px' }}>
                                {c.status}
                              </span>
                            </div>
